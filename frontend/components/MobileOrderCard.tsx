 ...
 * Ручной порядок карточек (сохраняется в БД для каждого пользователя, задача
 * 13.08.2026) перетаскивается всей карточкой (без отдельной ручки — задача
 * 14.08.2026). Длинный тап (activationConstraint delay/tolerance в
...
export default function MobileOrderCard({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: o.id,
    // role="group", а не дефолтный "button" из dnd-kit — внутри карточки уже
    // есть свои button (RowActions, StatusQuickChange, тап-переключатель),
    // вложенный role="button" был бы некорректной ARIA-вложенностью.
    attributes: { role: 'group', roleDescription: `Заявка №${o.id}, долгий тап перетаскивает карточку` },
  });
  const style = {
  ...
  function toggleKeyDown(e: React.KeyboardEvent) {
    ...
    >
      <div className="flex items-stretch">
        {/* Чекбокс — отдельный элемент вне области тапа-разворота (не внутри
            ...
            Виден только в режиме выбора (включается главным чекбоксом «Выбрать»
            в шапке десктопной таблицы — состояние общее для стола и карточек). */}
        {selectionMode && (
          <label
            className="flex items-center pl-4 pr-1 animate-check-in"
        ...
        {/* Не <button> намеренно: PointerSensor (RowDragSensor) игнорирует
            ...
            Space, aria-expanded) сохранена через role="button" + onKeyDown. */}
        <div
          role="button"
          tabIndex={0}
          onClick={() => setOpen((v) => !v)}
          ...
          aria-expanded={open}
          aria-label={`Заявка №${o.id}`}
          className={`flex-1 min-w-0 min-h-[44px] flex items-center justify-between gap-2 ${selectionMode ? 'pl-2' : 'pl-4'} pr-4 py-3 text-left cursor-pointer`}
        >
